...
- **房间管理** - 加入/离开房间，管理参与者和媒体轨道（信令协议 17）
- **WebRTC 集成** - 通过 [webrtc-java](https://github.com/devopvoid/webrtc-java) 实现音视频发布和订阅
- **屏幕共享** - 发布整个屏幕或指定应用窗口
- **订阅控制** - 按轨道订阅/退订、视频质量、分辨率和帧率控制
- **参与者更新** - metadata、名称、属性更新及变更事件
- **数据通道** - 可靠和非可靠数据消息传输
- **RPC** - 调用远端参与者方法（`performRpc`、`registerRpcMethod`）
- **数据流** - 分块文本/字节/文件流及 topic 处理器（`sendText`、`sendFile`）
- **聊天与转写** - 聊天消息、语音转写片段和 SIP DTMF 事件
- **端到端加密** - 数据通道的 AES-GCM 加密，支持密钥轮换
- **自动重连** - 指数退避的自动重连机制
- **网络监控** - 恢复令牌和网络变化检测
- **音频处理** - 回声消除、降噪、自动增益控制选项
- **统计与编解码** - WebRTC 统计信息和首选视频编解码器选择
