...
/**
 * Builder for SyncState message used during reconnection. Captures client state to sync with server
 * after resume.
 */
public class SyncStateBuilder {
  ...
  public SyncStateBuilder addPublishedTrack(
      String cid, String sid, String name, livekit.LivekitModels.TrackType type) {
    LivekitRtc.TrackPublishedResponse response =
        LivekitRtc.TrackPublishedResponse.newBuilder()
            .setCid(cid)
            .setTrack(
                livekit.LivekitModels.TrackInfo.newBuilder()
                    .setSid(sid)
                    .setName(name)
                    .setType(type)
                    .build())
  ...
  public SyncStateBuilder addDataChannel(String label, int id, LivekitRtc.SignalTarget target) {
    LivekitRtc.DataChannelInfo info =
        LivekitRtc.DataChannelInfo.newBuilder().setLabel(label).setId(id).setTarget(target).build();
  ...
  public LivekitRtc.SyncState build() {
    ...
    if (!subscribedTrackSids.isEmpty()) {
      builder.setSubscription(
          LivekitRtc.UpdateSubscription.newBuilder()
    ...
    for (Map.Entry<String, Integer> entry : dataChannelReceiveStates.entrySet()) {
      builder.addDatachannelReceiveStates(
          LivekitRtc.DataChannelReceiveState.newBuilder()
